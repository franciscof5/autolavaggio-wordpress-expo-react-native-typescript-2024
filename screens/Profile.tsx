import React, {useState,} from 'react';
import {
    StatusBar,
  StyleSheet,
  Text,
  View,
  TextInput,
  Button,
  TouchableOpacity,
} from "react-native";
// import {ImageOrVideo} from 'react-native-image-crop-picker';
import {Avatar} from './Avatar';
import {UserInfo} from './UserInfo';
import axios from 'axios';
import * as ImagePicker from 'expo-image-picker';
import * as FileSystem from 'expo-file-system';
import { useForm } from 'react-hook-form';
const logo = require("../assets/images/gio-logo.png")
// import { uploadFiles, DocumentDirectoryPath } from "react-native-fs"; //NUNCA VAI
// import RNFetchBlob from 'rn-fetch-blob'
// import * as DocumentPicker from 'expo-document-picker';
// import FileBase64 from 'react-file-base64';
// import ImgToBase64 from 'react-native-image-base64';
import { useSelector } from 'react-redux'
// import { currentUserSlice } from '../api/currentUserApi/currentUserSlice';

export default function Profile() {
  const [image, setImage] = useState(null);
  const userObject = useSelector((state) => Object.values(state.currentUserApi.mutations)[0].data )
  const onAvatarChange = async (image) => {
    const fil = image.assets[0].uri;
    const base64 = await FileSystem.readAsStringAsync(fil, { encoding: 'base64' });
    console.log(base64);
    return;
    const formData = new FormData();
    formData.append('token', userObject.token);
    formData.append('image', base64);

    var options = {
        method: 'POST',
        url: 'https://www.lavaggioapp.it/wp-json/myplugin/v1/uploadAvatar',
        params: {'': ['', '']},
        headers: {
            'Content-Type': `multipart/form-data; boundary=${formData._boundary}`
        //   'content-type': 'multipart/form-data; boundary=---011000010111000001101001'
        },
        data: formData,
        // data: '-----011000010111000001101001\r\nContent-Disposition: form-data; name="token"\r\n\r\neyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjMxMDk2MSwibmJmIjoxNzIyMzEwOTYxLCJleHAiOjE3MjI5MTU3NjEsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.R1r3jYddInZxuytlY5H3cOtYtcNyQVIYBd9KWUn_KfU\r\n-----011000010111000001101001\r\nContent-Disposition: form-data; name="image"\r\n\r\\r\n-----011000010111000001101001--\r\n'
      };
      
    //   axios.post('https://www.lavaggioapp.it/wp-json/myplugin/v1/uploadAvatar', options)
    axios.post('https://www.lavaggioapp.it/wp-json/myplugin/v1/uploadAvatar', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      })  
    .then(function (response) {
        console.log(response.data);
      }).catch(function (error) {
        console.error(error);
      });


  };
  
  return (
    <View style={styles.scroll}>
      <StatusBar barStyle="dark-content" />
      <View style={styles.userRow}>
        <Text style={styles.file}>Upload CSV File</Text>
        <View style={styles.button}>
            <TouchableOpacity>
            <Button
                title="upload your file"
                color="black"
                onPress={onAvatarChange}
            />
            </TouchableOpacity>
        </View>
        <Avatar
          onChange={onAvatarChange}
          source={require('./../assets/avatar-placeholder.png')}
        />
        <UserInfo />
      </View>
      <View style={styles.content} />
    </View>
  );
};

const styles = StyleSheet.create({
  scroll: {
    backgroundColor: 'white',
    flex: 1,
  },
  userRow: {
    alignItems: 'center',
    padding: 15,
    marginTop: 70,
  },
  content: {
    flex: 1,
    backgroundColor: '#d8d8db',
  },
});

    // const FormData = require('form-data');
    // const fetch = require('node-fetch');
    // const formData = new FormData();

    // formData.append('token', 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjMxMDk2MSwibmJmIjoxNzIyMzEwOTYxLCJleHAiOjE3MjI5MTU3NjEsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.R1r3jYddInZxuytlY5H3cOtYtcNyQVIYBd9KWUn_KfU');
    // formData.append('image', '');

    // let url = 'https://www.lavaggioapp.it/wp-json/myplugin/v1/uploadAvatar';

    // let options = {
    // method: 'POST',
    // headers: {
    //     'content-type': 'multipart/form-data; boundary=---011000010111000001101001',
    // }
    // };

    // options.body = formData;

    // fetch(url, options)
    // .then(res => res.json())
    // .then(json => console.log(json))
    // .catch(err => console.error('error:' + err));

    // var files = [
    //     {
    //       name: "file",
    //       filename: "file.jpg",
    //       filepath: fil,
    //       filetype: "image/jpeg",
    //     },
    //   ];
    
    
    // let result = await DocumentPicker.getDocumentAsync({})
    // .then((result)=>{
    //     console.log("result", result.assets)
    //     const url = "https://www.lavaggioapp.it/wp-json/wp/v2/media";
    //     // const fileUri = result.uri;
    //     const formData = new FormData();
    //     formData.append('file', result.assets);
    //     const options = {
    //         method: 'POST',
    //         body: formData,
    //         headers: {
    //             'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjE4NTM4MiwibmJmIjoxNzIyMTg1MzgyLCJleHAiOjE3MjI3OTAxODIsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.RY52qyVoN2Ne_6EAudy-3Db0F7nR4qdqQZQhkP2xB5A',
    //             Accept: 'application/json',
    //             'Content-Type': 'multipart/form-data',
    //         },
    //     };
    //     console.log("FFFFF");
    //     console.log(formData);

    //     axios.post(url, options, {
    //         headers: {
    //             'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjE4NTM4MiwibmJmIjoxNzIyMTg1MzgyLCJleHAiOjE3MjI3OTAxODIsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.RY52qyVoN2Ne_6EAudy-3Db0F7nR4qdqQZQhkP2xB5A',
    //             Accept: 'application/json',
    //             'Content-Disposition': 'form-data; filename="stesddt_funn2y.jpg"',
    //             'Content-Type': 'multipart/form-data',
    //         },
    //     })
    //     .catch((error) => console.log(error));;

    //     // fetch(url, options)
    //     // .catch((error) => console.log(error));
    // });


    

    
    //   RNFetchBlob.fetch('POST', 'https://www.lavaggioapp.it/wp-json/wp/v2/media', {
    //     Authorization : "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjE4NTM4MiwibmJmIjoxNzIyMTg1MzgyLCJleHAiOjE3MjI3OTAxODIsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.RY52qyVoN2Ne_6EAudy-3Db0F7nR4qdqQZQhkP2xB5A",
    //     'Dropbox-API-Arg': JSON.stringify({
    //         path : fil,
    //         mode : 'add',
    //         autorename : true,
    //         mute : false
    //       }),
    //     // 'Dropbox-API-Arg': JSON.stringify({
    //     //   path : fil,
    //     //   mode : 'add',
    //     //   autorename : true,
    //     //   mute : false
    //     // }),
    //     // 'Content-Type' : 'application/octet-stream',
    //     // here's the body you're going to send, should be a BASE64 encoded string
    //     // (you can use "base64"(refer to the library 'mathiasbynens/base64') APIs to make one).
    //     // The data will be converted to "byte array"(say, blob) before request sent.
    //   })
    //   .then((res) => {
    //     console.log(res.text())
    //   })
    //   .catch((err) => {
    //     // error handling ..
    //   })

    
    // uploadFiles({
    //     toUrl: "https://www.lavaggioapp.it/wp-json/wp/v2/media",
    //     files: files,
    //     method: "POST",
    //     // headers: {
    //     //   Accept: "application/json",
    //     // },
    //     //     // url: 'https://www.lavaggioapp.it/wp-json/wp/v2/media',
    //     // headers: { 
    //     //     "Accept": "application/json",
    //     //     'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjE4NTM4MiwibmJmIjoxNzIyMTg1MzgyLCJleHAiOjE3MjI3OTAxODIsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.RY52qyVoN2Ne_6EAudy-3Db0F7nR4qdqQZQhkP2xB5A', 
    //     //     'Content-Disposition': 'form-data; filename="stesddt_funn2y.jpg"', 
    //     //     'Content-Type': 'image/png', 
    //     //     'Cookie': 'PHPSESSID=hq9es29n40rdkdv0i6jukoe8jc'
    //     // },
    //     //invoked when the uploading starts.
    //     begin: () => {},
    //     // You can use this callback to show a progress indicator.
    //     progress: ({ totalBytesSent, totalBytesExpectedToSend }) => {},
    //   });

    // const myHeaders = new Headers();
    // myHeaders.append("token", "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjMxMDk2MSwibmJmIjoxNzIyMzEwOTYxLCJleHAiOjE3MjI5MTU3NjEsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.R1r3jYddInZxuytlY5H3cOtYtcNyQVIYBd9KWUn_KfU");
    // myHeaders.append("Content-Disposition", "form-data; filename=\"stest_funn2y.jpg\"");
    // myHeaders.append("Content-Type", "image/png");
    // myHeaders.append("Cookie", "PHPSESSID=hq9es29n40rdkdv0i6jukoe8jc");

    // const file = image;

    // const requestOptions = {
    //     method: "POST",
    //     headers: myHeaders,
    //     body: file,
    //     redirect: "follow"
    // };

    // fetch("https://www.lavaggioapp.it/wp-json/myplugin/v1/uploadAvatar", requestOptions)
    // .then((response) => response.text())
    // .then((result) => console.log("result", result))
    // .catch((error) => console.error("error", error));

    //#

    // let data = logo;
    // const base64 = await FileSystem.getContentUriAsync(image.assets[0].uri)
    // var Buffer = require('buffer/').Buffer
    // const buffer = Buffer.from(base64, 'base64')
    // var blob = new Blob([ base64 ])
    
    // var xhr = new XMLHttpRequest();
    // xhr.withCredentials = true;

    // xhr.addEventListener("readystatechange", function() {
    // if(this.readyState === 4) {
    //     console.log(this.responseText);
    // }
    // });

    // xhr.open("POST", "https://www.lavaggioapp.it/wp-json/wp/v2/media");
    // xhr.setRequestHeader("Authorization", "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjE4NTM4MiwibmJmIjoxNzIyMTg1MzgyLCJleHAiOjE3MjI3OTAxODIsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.RY52qyVoN2Ne_6EAudy-3Db0F7nR4qdqQZQhkP2xB5A");
    // xhr.setRequestHeader("Content-Disposition", "form-data; filename='stest_funn2y.jpg'");
    // xhr.setRequestHeader("Content-Type", "image/jpg");
    // // WARNING: Cookies will be stripped away by the browser before sending the request.
    // xhr.setRequestHeader("Cookie", "PHPSESSID=hq9es29n40rdkdv0i6jukoe8jc");

    // xhr.send(blob);

    // let config = {
    // method: 'post',
    // maxBodyLength: Infinity,
    // url: 'https://www.lavaggioapp.it/wp-json/wp/v2/media',
    // headers: { 
    //     'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjE4NTM4MiwibmJmIjoxNzIyMTg1MzgyLCJleHAiOjE3MjI3OTAxODIsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.RY52qyVoN2Ne_6EAudy-3Db0F7nR4qdqQZQhkP2xB5A', 
    //     'Content-Disposition': 'form-data; filename="stest_funn2y.jpg"', 
    //     'Content-Type': 'image/png', 
    //     'Cookie': 'PHPSESSID=hq9es29n40rdkdv0i6jukoe8jc'
    // },
    // data : image.assets[0].uri
    // };

    // axios.request(config)
    // .then((response) => {
    // console.log(response.data);
    // })
    // .catch((error) => {
    // console.log(error);
    // });


    // let data = '<file contents here>';
    // const base64 = await FileSystem.getContentUriAsync(image.assets[0].uri)
    // const buffer = Buffer.from(base64, 'base64')

    // let config = {
    // method: 'post',
    // maxBodyLength: Infinity,
    // url: 'https://www.lavaggioapp.it/wp-json/wp/v2/media',
    // headers: { 
    //     'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjE4NTM4MiwibmJmIjoxNzIyMTg1MzgyLCJleHAiOjE3MjI3OTAxODIsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.RY52qyVoN2Ne_6EAudy-3Db0F7nR4qdqQZQhkP2xB5A', 
    //     'Content-Disposition': 'form-data; filename="stest_funn2y.jpg"', 
    //     'Content-Type': 'image/png', 
    //     'Cookie': 'PHPSESSID=hq9es29n40rdkdv0i6jukoe8jc'
    // },
    // data : buffer
    // };

    // axios.request(config)
    // .then((response) => {
    // console.log(JSON.stringify(response.data));
    // })
    // .catch((error) => {
    // console.log(error);
    // });


    // const fetch = require('node-fetch');

    // let formdata = new FormData()

    // formdata.append("product[name]", 'test')
    // formdata.append("product[price]", 10)
    // formdata.append("product[category_ids][]", 2)
    
    // formdata.append("product[description]", '12dsadadsa')
    
    // formdata.append("image", {
    //     "uri": image.assets[0].uri, 
    //     "name": 'image.jpg', 
    //     "type": 'image/jpeg'
    // })
    // console.log(image.assets[0].uri)
    // // formdata.append("product[images_attributes[0][file]]", image.assets[0].uri);

    // let url = 'https://www.lavaggioapp.it/wp-json/wp/v2/media';
    // let options = {
    //     method: 'POST',
    //     headers: {
    //         'Content-Type': 'multipart/form-data',
    //         Authorization: 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL3d3dy5sYXZhZ2dpb2FwcC5pdCIsImlhdCI6MTcyMjE4NTM4MiwibmJmIjoxNzIyMTg1MzgyLCJleHAiOjE3MjI3OTAxODIsImRhdGEiOnsidXNlciI6eyJpZCI6IjE4OSJ9fX0.RY52qyVoN2Ne_6EAudy-3Db0F7nR4qdqQZQhkP2xB5A',
    //         // 'Content-Disposition': 'attachment; filename="test_funn2y.jpg"',
    //         // "Content-Disposition": "form-data",
    //         "name":"async-upload",
    //         // "filename":"bg.png",
    //         // "Accept": "application/json",
    //         // cookie: 'PHPSESSID=efl5pir6pnfu61349l1p22ib7b; ',
    //         // "cache-control": "no-cache"
    //     },
    //     body: serializeJSON({
    //         haha: formdata
    //       }),
    // };

    // fetch(url, options)
    // .then(res => res.json())
    // .then(json => console.log(json))
    // .catch(err => console.error('error:' + err));
    // upload image to server here 